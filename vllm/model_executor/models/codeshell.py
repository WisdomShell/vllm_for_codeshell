...
class CodeShellBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = CodeShellAttention(config, cache_config, quant_config)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.mlp = CodeShellMLP(inner_dim, config, quant_config)
...
class CodeShellModel(nn.Module):
    ...
    def __init__(
        ...
        self.h = nn.ModuleList([
            CodeShellBlock(config, cache_config, quant_config)
    ...
    def forward(
        ...
        for i in range(len(self.h)):
            layer = self.h[i]
            hidden_states = layer(hidden_states=hidden_states, positions=position_ids, 
                                  kv_cache=kv_caches[i], attn_metadata=attn_metadata)
